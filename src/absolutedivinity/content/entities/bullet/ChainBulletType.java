...
public class ChainBulletType extends BulletType {
    ...
    public ChainBulletType(float damage, float charge, Color color) {
        this.damage = damage;
    ...
    public void update(Bullet b) {
        ...
        for (int i = 0; i < helixCount; i++) {
            ...
            trails[i].update(
                b.x + Mathf.cosDeg(ang) * helixRadius,
                b.y + Mathf.sinDeg(ang) * helixRadius,
                b.fout()
            );
        }
    }
    ...
    public void draw(Bullet b) {
        ...
        float z = Draw.z();
        Draw.z(z - 0.0001f);
        for (Trail t : trails) t.draw(bulletColor, helixWidth * fade);
        Draw.z(z);

        float[][] circles = {
            {bulletColor.r, bulletColor.g, bulletColor.b, 0.2f * fade, helixRadius * 1.6f * pulse},
            {bulletColor.r, bulletColor.g, bulletColor.b, 0.55f * fade, helixRadius * 0.9f * pulse},
            {glowColor.r, glowColor.g, glowColor.b,         fade, helixRadius * 0.45f * pulse}
        };
        for (float[] c : circles) {
            Draw.color(c[0], c[1], c[2], c[3]);
            Fill.circle(b.x, b.y, c[4]);
        }
